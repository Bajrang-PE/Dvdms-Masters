...
export const getCommonHpFinYearCmb = async (hospitalCode, size) => {
    try {
        const response = await fetchData(
            `/hp-po-api/common/fy-combo?hospitalCode=${hospitalCode}&size=${size}`
        );
        return response.data;
    } catch (err) {
        console.error("API Error : ", err);
...
export const getCommonHpStoreDetailsCmb = async (hospitalCode, storeId) => {
    try {
        const response = await fetchData(
            `/hp-po-api/stores/details/${storeId}?hospitalCode=${hospitalCode}`
        );
        return response.data;
    } catch (err) {
        console.error("API Error : ", err);
...
export const getCommonHpBudgetDetailsCmb = async (hospitalCode, storeId, fundingId, prgId, finYear) => {
    try {
        const response = await fetchData(
            `/hp-po-api/budget/budget-details?hospitalCode=${hospitalCode}&fundingSourceId=${fundingId}&programmeId=${prgId}&storeId=${storeId}&financialYear=${finYear}`
        );
        return response.data;
    } catch (err) {
        console.error("API Error : ", err);
...
export const getCommonHpUnitCombo = async (hospitalCode) => {
    try {
        const response = await fetchData(`/hp-po-api/unit/combo?hospitalCode=${hospitalCode}`);
        return response?.data;
    } catch (error) {
...
export const getCommonHpSupplierCombo = async (hospitalCode) => {
    try {
        const response = await fetchData(`/hp-po-api/rate-contracts/supplier-combo?hospitalCode=${hospitalCode}&status=1`);
        return response?.data;
    } catch (error) {
        console.error('API Error', error);
        throw error;
...
export const getCommonHpApprByCmbCombo = async (hospitalCode, storeId) => {
    try {
        const response = await fetchData(`/hp-po-api/stores/approved-by-combo?hospitalCode=${hospitalCode}&storeId=${storeId}`);
        return response?.data;
    } catch (error) {
        console.error('API Error', error);
        throw error;
