...
export default function ModifyBranchForm({ data }) {
  //redux states
  const dispatch = useDispatch();
  const {
    gnumBranchId: branchID,
  ...
  console.log('Modify Form Rendered : ', data);

  // Controlled state for inputs
  const [branchName, setBrachName] = useState(gstrBranchName);
  const [branchShortName, setBranchShortName] = useState(gstrBranchShortName);
  const [ifscCode, setIfscCode] = useState(gstrIfscCode);
  const [bankAddress, setBankAddress] = useState(gstrBranchAddress);
  const [selectedState, setSelectedState] = useState(gnumStateCode);
  const [selectedDistrict, setSelectedDistrict] = useState(gnumDistId);
  ...
  async function handleModify() {
    ...
    await modifyBranch(JSON.stringify(branchDetails), selectedBankID, branchID);
  ...
  return (
    <>
      <h3 className="branchmaster__heading">Modify Branch</h3>
