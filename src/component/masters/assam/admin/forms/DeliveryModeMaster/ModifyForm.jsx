...
import { modifyDelivery } from '../../../../../../api/deliveryModeMasterAPI';

const specialCharacterCheck = /[^a-zA-Z0-9\s]/g;

export default function ModifyDeliveryForm({ data }) {
  const dispatch = useDispatch();

  console.log(data);

  const {
    hstnumDeliverymodeId,
    gstrRemarks,
    gdtEffectiveFrm,
    hststrDeliverymodeName,
    gnumIsvalid,
  } = data.at(0);

  // Controlled state for inputs
  const [deliveryMode, setDeliveryMode] = useState(hststrDeliverymodeName);
  ...
  async function handleSave() {
    ...
    try {
      await modifyDelivery(JSON.stringify(deliveryDetails));
      alert('Delivery Details Updated Succesfully!');
      location.reload();
    } catch (err) {
  ...
  return (
    <>
      <h3 className="bankmaster__heading">Modify Delivery Mode</h3>
